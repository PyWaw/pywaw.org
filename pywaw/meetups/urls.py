...
urlpatterns = [
    ...
    url(r'^zgloszenie/potwierdzenie/$', views.TalkProposalConfirmationView.as_view(),
        name='talk_proposal_confirmation'),
    ...
    url(r'^(?P<day>\d{2})-(?P<month>\d{2})-(?P<year>\d{4})/$', views.MeetupDateRedirectView.as_view(permanent=True),
        name='date_redirect'),
    url(r'^home/sponsor/$', generic.RedirectView.as_view(url=reverse_lazy('meetups:sponsor_list'), permanent=True)),
    url(r'^home/poradnik-prelegenta$',
        generic.RedirectView.as_view(url=reverse_lazy('meetups:speaker_list'), permanent=True)),
]
